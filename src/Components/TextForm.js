...
export default function TextForm(props) {
  ...
  const handleUpClick = () => {
    // console.log("UpperCase was clicked" + text);     //for console purpose only.
  ...
  const handleOnChange = (event) => {
    // console.log("OnGoing change"); -->> not imp,we used for developer tools to fix bugs
  ...
  return (
    ...
    <div className="container" style={{color:props.mode==='dark'?'white':'black'}}>
      ...
      <div className="mb-3">
        <textarea
          ...
          style={{backgroundColor:props.mode==='dark'?'#3d474f':'#dedcdc',color:props.mode==='dark'?'white':'black' }}
          id="myBox"
    ...
    <h2>Your text summar</h2>
    <p> {text.split(/\s+/).filter((element)=>{return element.length!==0}).length} words and {text.length} characters</p>
    <p>{0.008*text.split(" ").filter((element)=>{return element.length!==0}).length} Minutes read</p>
    <h2>Preview</h2>
    <p>{text.length>0?text:"Nothing to preview! ⬆️."}</p>
    </div>
    </>
  );
}
